...
BEGIN
  SET @QUERY =    "SELECT  
   ...
   IF (type=1 OR type='1') THEN 
      SET @QUERY = CONCAT(@QUERY, " `jt`.`name` as `selection` FROM `vl_site_age` `va` JOIN `agecategory` `ac` ON `ac`.`ID` = `va`.`age` JOIN `view_facilitys` `jt` ON `jt`.`id` = `va`.`facility` WHERE 1 ");
   END IF;
