...
class Nonsuppression extends MY_Controller
{
	...
	function gender_group($year=NULL,$month=NULL,$county=NULL)
	{
		$data['suppressions'] = $this->nonsuppression_model->gender_group_chart($year,$month,$county);

    	$this->load->view('sup_gender_group_view',$data);
	}

	function age_group($year=NULL,$month=NULL,$county=NULL)
	{
		$data['suppressions'] = $this->nonsuppression_model->age_group_chart($year,$month,$county);
	...
	{
		$data['suppressions'] = $this->nonsuppression_model->justifications($year,$month,$county);

    	$this->load->view('sup_justification_view',$data);
	}
	function regimen($year=NULL,$month=NULL,$county=NULL)
	{
		$data['suppressions'] = $this->nonsuppression_model->regimen($year,$month,$county);

    	$this->load->view('sup_regimen_view',$data);
	}

	function sample_type($year=NULL,$month=NULL,$county=NULL)
	{
		$data['suppressions'] = $this->nonsuppression_model->sampletypes($year,$month,$county);
